type=page
status=published
title=The Lifecycle of a Jakarta Faces Application
...
two main phases of a Jakarta Faces web application, is introduced in
link:jsf-intro004.html#GJAAM[A Simple Jakarta Faces Application]. This
section examines the Jakarta Faces lifecycle in more detail.
...
view. During the lifecycle, the Jakarta Faces implementation must
...
.*Figure 7-3 Jakarta Faces Standard Request-Response Lifecycle*
...
Jakarta Faces page. To render a response that is another Jakarta Server
...
does not contain Jakarta Faces components. In these situations, the
...
request for another Jakarta Faces page. In this case, the Jakarta Server
...
the lifecycle phases, download the latest Jakarta Faces Specification
...
When a request for a Jakarta Faces page is made, usually by an
action, such as when a link or a button component is clicked, the
Jakarta Faces implementation begins the Restore View phase.

During this phase, the Jakarta Faces implementation builds the view
...
Jakarta Faces implementation adds an error message to the
