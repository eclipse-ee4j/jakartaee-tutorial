type=page
status=published
title=What Is a Jakarta Faces Application?
...
The functionality provided by a Jakarta Faces application is similar
to that of any other Java web application. A typical Jakarta Faces
...
objects (POJOs). In a Jakarta Faces application, managed beans serve
...
in a typical Jakarta Faces application. In response to a client
request, a web page is rendered by the web container that implements
Jakarta Faces technology.

[[BNAPI]]

.*Figure 7-1 Responding to a Client Request for a Jakarta Faces Page*
...
The web page, `myfacelet.xhtml`, is built using Jakarta Faces
