...
[[BABGECCJ]][[html5-friendly-markup]]

HTML5-Friendly Markup
---------------------

When you want to produce user interface features for which HTML does not
have its own elements, you can create a custom Jakarta Faces
...
existing components. Jakarta Faces technology supports HTML5 not by
...
can mix and match Jakarta Faces and HTML5 components and elements as
you see fit.
...
treat them as equivalent to Jakarta Faces components associated with
...
pass-through attribute in a Jakarta Faces `UIComponent`, the
attribute name and value are passed straight through to the browser
without being interpreted by Jakarta Faces components or renderers.
There are several ways to specify pass-through attributes.

* Use the Jakarta Faces namespace for pass-through attributes to
prefix the attribute names within a Jakarta Faces component. For
...
to the browser uninterpreted by the Jakarta Faces input component:
...
"Using Ajax with Jakarta Faces Technology"], for more information.
