...
`_tut-install_/examples/web/websocket/dukeetf2/` directory,
demonstrates how to use a WebSocket endpoint to provide data updates to
web clients. The example resembles a service that provides periodic
updates on the price and trading volume of an electronically traded
fund (ETF).
...
in the timeout method. See <<using-the-timer-service>> in xref:
<<running-the-enterprise-bean-examples>> for more information on the
timer service.
...
The WebSocket API is supported by most modern browsers, and it is
widely used in HTML5 web client development.
...
. Click *Open Project*.
. In the *Projects* tab, right-click the `dukeetf2` project and select
*Run*.
+
