...
from a checking account to a savings account by using the steps listed
...
crash during the `credit` step, the transaction would roll back and
undo the data modifications made by the `debit` statement. Although the
