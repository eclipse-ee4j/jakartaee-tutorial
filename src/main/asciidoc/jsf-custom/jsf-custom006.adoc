...
Jakarta Faces implementation as a renderer class. The annotation
...
By submitting the page, this code causes the Jakarta Faces lifecycle to
return back to the Restore View phase. This phase saves any state
...
called by the Jakarta Faces implementation during the Apply Request
...
calls the `getRendererType` method of the component's tag handler to
determine which renderer to invoke, if there is one.
