...
implementing this kind of image map because it can perform the work
that must be done on the server without requiring you to create a
...
server to retrieve data or to change the appearance of nonform
controls, tasks that a client-side image map cannot do.

Because the image map custom component uses Jakarta Server Faces
technology, it has the best of both styles of image maps: It can handle
the parts of the application that need to be performed on the server
while allowing the other parts of the application to be performed on
the client side.
...
The `map` tag specifies the image map and contains a set of `area`
tags.
...
of a region, the `onmouseout` function redisplays the original image.
If the user clicks on a region, the `onclick` function sets the value
of the `input` tag to the ID of the selected area and submits the page.

The `input` tag represents a hidden control that stores the value of
the currently selected area between client-server exchanges so that the
...
the locale in the `jakarta.faces.context.FacesContext` instance
according to the region that was selected.

=== Understanding the Facelets Page

Here is an abbreviated form of the Facelets page that the image map
component uses to generate the HTML page shown in the preceding
section. It uses custom `bookstore:map` and `bookstore:area` tags to
represent the custom components:
...
information on navigation, see <<configuring-navigation-rules>>.
...
The `current` attribute of the `bookstore:map` tag is set to the
default area, which is `map1` (the book My Early Years: Growing Up on
Star7, by Duke).
...
`onmouseout` attribute values indicate the image to be loaded when
these events occur. How the JavaScript is generated is explained more
in <<performing-encoding>>.
...
scope. The value of this attribute is a bean, which stores the
`coords`, `shape`, and `alt` data. How these beans are stored in the
application scope is explained more in the next section.

=== Configuring Model Data

In a Jakarta Server Faces application, data such as the coordinates of
a hotspot of an image map is retrieved from the `value` attribute
through a bean. However, the shape and coordinates of a hotspot should
be defined together because the coordinates are interpreted differently
...
initialized into application scope by the managed bean creation
facility (see <<using-the-managed-bean-element>>). Here is part of the
managed bean declaration for the `ImageArea` bean corresponding to the
...
For more information on initializing managed beans with the managed
bean creation facility, see the section
...
To reference the `ImageArea` model object bean values from the
component class, you implement a `getValue` method in the component
class. This method calls `super.getValue`. The superclass of
`_tut-install_/examples/case-studies/dukes-bookstore/src/java/dukesbookstore/components/AreaComponent.java`,
...
of `ImageArea`. <<creating-the-renderer-class>> explains how to do this
in the `AreaRenderer` class.

=== Summary of the Image Map Application Classes

<<image-map-classes>> summarizes all the classes needed to implement
the image map component.

[[image-map-classes]]
[width="60%",cols="15%a,45%a",title="Image Map Classes"]
|===
|Class |Function

|`AreaSelectedEvent` |The `jakarta.faces.event.ActionEvent` indicating
...
|`AreaRenderer` |This `jakarta.faces.render.Renderer` performs the
...
`_bookstore-dir_/listeners/`. The component classes are located in
`_bookstore-dir_/components/`. The renderer classes are located in
`_bookstore-dir_/renderers/`. `ImageArea` is located in
`_bookstore-dir_/model/`.
