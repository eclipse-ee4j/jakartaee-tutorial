...
You can also create a managed bean property that accepts and returns the `Validator` implementation you create, as described in <<writing-properties-bound-to-converters-listeners-or-validators>>.
...
You can queue the error messages onto the `FacesContext` programmatically, or you can register the error messages in the application configuration resource file, as explained in <<registering-application-messages>>.
