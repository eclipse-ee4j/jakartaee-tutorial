...
with `VendorPart`{zwsp}'s `part` field. That is, each part has exactly one
...
`PARTREVISION` column refers to `PERSISTENCE_ORDER_PART`{zwsp}'s `REVISION`
column.
...
relationship with `LineItem`{zwsp}'s field `customerOrder`. That is, each
...
`@GeneratedValue`{zwsp}'s `strategy=TABLE` element. That is, the strategy used
...
`VendorPart`{zwsp}'s primary key will increment by 10.
...
* `Part` uses the `PartKey` wrapper class. `Part`{zwsp}'s primary key is a
combination of the part number and the revision number. `PartKey`
encapsulates this primary key.
* `LineItem` uses the `LineItemKey` class. `LineItem`{zwsp}'s primary key is a
...
In `LineItem`{zwsp}'s constructor, the line item number (`LineItem.itemId`) is
...
`Part`{zwsp}'s compound primary key, because `Part`{zwsp}'s compound primary key is
...
`Part`{zwsp}'s fields map to more than one database table:
...
`PERSISTENCE_ORDER_PART_DETAIL`{zwsp}'s primary key columns are foreign keys
...
type specified in `@Temporal`{zwsp}'s element. `@Temporal`{zwsp}'s elements are of
...
which is used to manage `order`{zwsp}'s entities in `RequestBean`{zwsp}'s business
methods:
...
creates the initial entities used by `order` by calling `RequestBean`{zwsp}'s
business methods.
...
file located at
`_tut-install_/examples/persistence/order/target/order.war`. Then the WAR
