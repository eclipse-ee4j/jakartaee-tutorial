...
To configure WebSockets for use in faces web applications, first enable
the WebSocket endpoint using the context parameter in `web.xml`:
...
Declare the `f:websocket tag` in the faces view with a channel name and
