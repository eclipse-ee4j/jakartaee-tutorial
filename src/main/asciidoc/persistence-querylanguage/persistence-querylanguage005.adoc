...
* See also: link:#BNBUM[Identification
Variables].
...
input parameter using a colon (`:`) followed by an identifier. The
first input parameter is `:position`, the second is `:name`.
...
an input parameter. The `AS` keyword in the `FROM` clause is optional.
In the `WHERE` clause, the period preceding the persistent variable
`city` is a delimiter, not a navigation operator. Strictly speaking,
...
checks whether the teams are associated with any leagues and returns
the teams that do not have a league.
