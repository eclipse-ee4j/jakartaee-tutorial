...
This section explains how components use the Jakarta Connectors Common
Client Interface (CCI) API and a resource adapter to access data from
an EIS. The CCI API defines a set of interfaces and classes whose
...
`ResultSet` instances, all of which inherit from the `Record`
interface.
...
adapter is very much like any other Jakarta EE client that uses
