...
of type Type of the entity, there is a getter method `get`{zwsp}Property and
setter method `set`{zwsp}Property. If the property is a Boolean, you may use
`is`{zwsp}Property instead of `get`{zwsp}Property. For example, if a `Customer`
