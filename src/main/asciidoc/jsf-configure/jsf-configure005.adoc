...
Faces application and to store them in scope. The managed bean creation
...
are Jakarta Faces managed beans, not CDI managed beans.
...
Faces implementation processes the `managed-bean` element. If a page
references the bean and no bean instance exists, the Jakarta Faces
implementation instantiates the bean as specified by the element
configuration.
