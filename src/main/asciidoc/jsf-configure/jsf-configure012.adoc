...
<<creating-custom-component-classes>>, or the `component` element of
...
If the custom component can include facets, you can configure the
facets in the component configuration using `facet` elements, which are
allowed after the `component-class` elements. See
<<registering-a-custom-renderer-with-a-render-kit>> for further details
on configuring facets.
