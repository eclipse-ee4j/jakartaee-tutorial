...
`jakarta.faces.validator.Validator` interface to perform validation,
you must register this custom validator either by using the
`@FacesValidator` annotation, as described in
<<implementing-the-validator-interface>>, or by using the `validator`
...
which the `Validator` class should be registered. This ID is used by
the tag class corresponding to the custom `validator` tag.
...
<<creating-and-using-a-custom-validator>> explains how to implement the
`Validator` interface.

<<using-a-custom-validator>> explains how to reference the validator
from the page.
