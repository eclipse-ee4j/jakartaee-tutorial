...
As explained in link:#BNAQM[Managed Beans in
...
bean properties. See link:#BNATG[Binding Component
...
objects described in link:#BNARF[Adding Components to a
...
link:#BNAUA[Table 12-1] lists the `jakarta.faces.component` classes and
...
for which an appropriate `jakarta.faces.convert.Converter` implementation
is available
...
As described in link:#BNAST[Using the Standard
...
link:#BNASV[Using DateTimeConverter], that displays
...
in link:#BNARZ[Using Data-Bound Table Components]. Here
...
link:#BNASE[Displaying Components for Selecting One
...
link:#BNASI[Displaying Components for Selecting Multiple
Values]:
...
link:#BNASH[Displaying a Menu Using the h:selectOneMenu
Tag]:
...
`jakarta.faces.model.SelectItem`. A `SelectItem` object is composed of an
