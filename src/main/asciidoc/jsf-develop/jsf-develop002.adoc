...
Here is the example tag, from <<using-datetimeconverter>>, that displays the date on which items will be shipped.
...
`UIData` components must be bound to one of the managed bean property types listed in <<acceptable-types-of-component-values>>.
...
See <<uiselectitems-properties>> for information on writing the bean properties for the `UISelectItem` and `UISelectItems` components.
...
Here is an example of the `h:selectOneMenu` tag from <<displaying-a-menu-using-the-hselectonemenu-tag>>:
...
This is explained in <<displaying-a-menu-using-the-hselectonemenu-tag>>.
...
The example `selectOneMenu` tag from <<uiselectone-properties>> contains `selectItem` tags that set the values of the list of items in the page.
