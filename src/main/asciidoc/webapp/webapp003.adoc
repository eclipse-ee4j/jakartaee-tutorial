...
The `hello1` application is a web module that uses Jakarta Faces
...
. Expand the *Web Pages* node and double-click the `index.xhtml` file to
...
. Expand the Source Packages node, then the `ee.jakarta.tutorial.hello1`
node. 

. [[hello1-nb-step-7, Step 7]]
+
...
. Under the Web Pages node, expand the WEB-INF node and double-click
...
addition, Jakarta Faces technology and Jakarta Servlet technology
...
The most commonly used scopes in Jakarta Faces applications are the
following:
...
For more information on scopes in Jakarta Faces technology, see
...
. In the *Projects* tab, right-click the `hello1` project and select
...
. To delete the class files and other build artifacts, go back to the
...
. To delete the class files and other build artifacts, enter the
following command:
+
[source,shell]
mvn clean
