...
In a terminal window, go to the `hello-faces` folder containing the `pom.xml` and execute the following command:
...
Find the `target/hello-faces.war` file and deploy it to a servlet container as instructed in <<Deploying The Example Projects>>.
