...
The source code for this application is in the `_tut-install_/tutorial/web/servlet/hello-servlet/` directory.
...
In a terminal window, go to the `hello-servlet` folder containing the `pom.xml` and execute the following command:
...
Find the `target/hello-servlet.war` file and deploy it to a servlet container as instructed in <<Deploying The Example Projects>>.
