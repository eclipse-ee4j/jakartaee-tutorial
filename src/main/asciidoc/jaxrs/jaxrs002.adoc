...
Java programming language type, or a `jakarta.ws.rs.core.Response` object.
...
|`jakarta.xml.bind.JAXBElement` and application-supplied Jakarta XML Binding classes |XML
...
are specified by using the `jakarta.ws.rs.QueryParam` annotation in the
...
`jakarta.ws.rs.CookieParam`, extract information from the cookies declared
in cookie-related HTTP headers. Header parameters, indicated by
decorating the parameter with `jakarta.ws.rs.HeaderParam`, extract
