...
Jakarta REST supports Bean Validation to verify Jakarta REST resource
classes. This support consists of:
...
throw a `jakarta.validation.ValidationException`.
...
method parameters in a resource class. To validate these entity
classes, use the `@Valid` annotation on the method parameter. For
example, the following class is a user-defined class containing both
standard and user-defined validation constraints.
...
runtime. Additional user-defined constraints can also trigger
...
the returned entity class as well as the `@PhoneRequired` and
`@NotNull` constraints defined within the entity class.

=== Validation Exception Handling and Response Codes

If a `jakarta.validation.ValidationException` or any subclass of
...
If a `ConstraintValidationException` is thrown, the Jakarta REST
runtime will respond to the client with one of the following HTTP
status codes:
