...
As mentioned in the previous section, Jakarta Faces technology supports
Ajax by using a built-in JavaScript resource library that is provided
as part of the Jakarta Faces core libraries. This built-in Ajax
resource can be used in Jakarta Faces web applications in one of the
following ways.
...
The `f:ajax` tag is a Jakarta Faces core tag that provides Ajax
