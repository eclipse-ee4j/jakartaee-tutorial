...
link:#BCGDJDFB[Packaging Applications] for basic
...
`jakarta.jms.ConnectionFactory`. To create multiple connection factories,
...
in link:#BABBABFC[Sending and Receiving Messages
Using a Simple Web Application], link:#BNCGW[Sending
...
link:#BNCEP[Jakarta Messaging Message Consumers].
...
`jakarta.transaction.UserTransaction` interface's transaction demarcation
...
The sections link:#GIPKO[What Is a Message-Driven Bean?]
and link:#BNCDW[How Does Jakarta Messaging Work with the
...
`jakarta.jms.MessageListener` interface, which means that it must provide
...
link:#BNCGV[Chapter 49, "Java Message Service Examples"]
...
`Durable`; see link:#BNCGD[Creating Durable
...
link:#BNCER[Jakarta Messaging Message Selectors] for information
...
transactions (described in link:#BNCGH[Using Jakarta Messaging
...
link:#BNCIK[Transaction Attributes] for more
information.
