...
. To log in as the user you created, or as one of two users already in
...
. On the page that appears, click one of the categories (Plants, Food,
Services, or Tools).
...
. When you have finished placing orders, click Logout at the top of
the page.
...
Duke's Store again as the customer, it will appear in the Orders list
as "Order cancelled."
...
. On the Upload the Product Image page, click Browse to locate an
image on your file system using a file chooser.
. Click Next.
. On the next page, view the product fields, then click Done.
. Click Products in the left sidebar, then click the category to verify
that the product has been added.
. Click Administration at the top of the page to return to the main
