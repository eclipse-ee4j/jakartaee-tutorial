...
The `jakarta.persistence.NamedEntityGraph` annotation defines a single
...
them within a `jakarta.persistence.NamedEntityGraphs` class-level
annotation.
...
entity graph used in the operation must be defined in the
`EmailMessage` class:
...
The attributes of a named entity graph are the fields of the entity
that should be included in the entity graph. Add the fields to the
entity graph by specifying them in the `attributeNodes` element of
`@NamedEntityGraph` with a `jakarta.persistence.NamedAttributeNode`
annotation:
