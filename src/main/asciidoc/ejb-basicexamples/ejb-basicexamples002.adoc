...
annotated with the `jakarta.ejb.Remote` annotation; the `jakarta.ejb.Local`
...
* `jakarta.ejb.PostActivate`: Methods annotated with `@PostActivate` are
invoked by the container after the container moves the bean from
secondary storage to active status.
* `jakarta.ejb.PrePassivate`: Methods annotated with `@PrePassivate` are
...
a database, a business method should throw a `jakarta.ejb.EJBException`.
