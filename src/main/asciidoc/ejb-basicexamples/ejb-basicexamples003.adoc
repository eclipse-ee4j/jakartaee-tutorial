...
`CounterBean`, and a Jakarta Faces Facelets web front end.
...
The web front end of `counter` consists of a Jakarta Faces managed
bean, `Count.java`, that is used by the Facelets XHTML files
`template.xhtml` and `index.xhtml`. The `Count` Jakarta Faces managed
...
getter method is called from the XHTML files, ``CounterBean``'s
`getHits` method is called to return the current number of page hits.
