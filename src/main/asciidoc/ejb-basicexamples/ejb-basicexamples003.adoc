...
implementation class is annotated with the `jakarta.ejb.Startup`
...
`jakarta.ejb.Lock` annotation and a `jakarta.ejb.LockType` type are used to
...
the Enterprise Bean container throws a `jakarta.ejb.ConcurrentAccessTimeoutException`.
The `jakarta.ejb.AccessTimeout` annotation is used to specify the number
