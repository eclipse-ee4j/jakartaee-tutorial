...
`CounterBean`, and a Jakarta Server Faces Facelets web front end.
...
The web front end of `counter` consists of a Jakarta Server Faces
managed bean, `Count.java`, that is used by the Facelets XHTML files
`template.xhtml` and `index.xhtml`. The `Count` Jakarta Server Faces
...
``CounterBean``'s `getHits` method is called to return the current
number of page hits.
