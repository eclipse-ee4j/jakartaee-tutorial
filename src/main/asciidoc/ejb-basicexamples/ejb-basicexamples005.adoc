...
greater than `y` is equivalent to the expression
`x-_maximumvalue_`,`_minimumvalue_-y`. That is, the expression begins
at `x`, rolls over to the beginning of the allowable values, and
continues up to `y`.
