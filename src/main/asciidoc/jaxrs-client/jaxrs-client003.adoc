...
`jakarta.ws.rs.core.MultivaluedMap` instance with the name-value pairs of
...
`jakarta.ws.rs.client.ClientResponseFilter` interfaces, respectively. Both
of these interfaces define a single method, `filter`. All filters must
be annotated with `jakarta.ws.rs.ext.Provider`.
