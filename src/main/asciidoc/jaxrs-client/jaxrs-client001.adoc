...
`jakarta.ws.rs.client.ClientBuilder.newClient` method. Then, the request
...
`jakarta.ws.rs.client.WebTarget.request` method sets the media type for
the returned entity. The `jakarta.ws.rs.client.Invocation.Builder.get`
...
MIME type or using one of the constants in `jakarta.ws.rs.core.MediaType`.
The `WebTarget.request` method returns an instance of
`jakarta.ws.rs.client.Invocation.Builder`, a helper object that provides
...
`jakarta.ws.rs.core.GenericType<T>` as the response type parameter, where
