...
For information on creating Jakarta Persistence entities, see xref:introduction-to-jakarta-persistence[].
For information on validating entity data, see <<validating-persistent-fields-and-properties>> and xref:bean-validation-advanced-topics[].
...
For information on creating and using enterprise beans, see xref:enterprise-beans[].
For information on creating strongly typed Criteria API queries, see xref:using-the-criteria-api-to-create-queries[].
...
For information on using Facelets, see xref:introduction-to-facelets[].
