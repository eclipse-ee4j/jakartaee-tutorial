...
For details, see <<_creating_durable_subscriptions>>, <<_creating_temporary_destinations>>, and <<_using_jakarta_messaging_local_transactions>>.
...
Some of the features work differently in the Jakarta EE web or enterprise bean container; in these cases, the differences are noted here and are explained in detail in <<_using_jakarta_messaging_in_jakarta_ee_applications>>.
...
(See <<_creating_durable_subscriptions>>.)
...
Instead, you use Jakarta Transactions, described in <<_using_jakarta_messaging_in_jakarta_ee_applications>>.
