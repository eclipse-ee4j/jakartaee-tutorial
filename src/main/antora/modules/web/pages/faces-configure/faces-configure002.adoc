...
Jakarta Faces support for bean annotations is introduced in xref:jsf-intro/jsf-intro.adoc#_jakarta_faces_technology[Jakarta Faces Technology].
...
These are discussed, along with registration of custom listeners, custom validators, and custom converters, in xref:jsf-custom/jsf-custom.adoc#_creating_custom_ui_components_and_other_custom_objects[Creating Custom UI Components and Other Custom Objects].
...
See xref:jsf-configure/jsf-configure.adoc#_using_faces_flows[Using Faces Flows] for more information.
