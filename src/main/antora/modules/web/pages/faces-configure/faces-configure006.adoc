...
A Jakarta Faces page uses the `message` or `messages` tags to access error messages, as explained in xref:jsf-page/jsf-page.adoc#_displaying_error_messages_with_the_hmessage_and_hmessages_tags[Displaying Error Messages with the h:message and h:messages Tags].
