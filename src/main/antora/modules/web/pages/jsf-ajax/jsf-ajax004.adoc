...
The application uses the attributes of the `f:ajax` tag listed in xref:jsf-ajax/jsf-ajax.adoc#_attributes_of_the_fajax_tag[attributes of the f:ajax tag] to create the Ajax request.
...
In the following code from the `reservation` example application (see xref:jsf-facelets/jsf-facelets.adoc#_the_reservation_example_application[The reservation Example Application]), a `listener` attribute is defined by an `f:ajax` tag, which refers to a method from the bean:
