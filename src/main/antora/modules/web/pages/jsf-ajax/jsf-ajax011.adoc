...
To demonstrate the advantages of using Ajax, revisit the `guessnumber` example from xref:jsf-facelets/jsf-facelets.adoc#_introduction_to_facelets[Introduction to Facelets].
...
The source code for this application is in the `_tut-install_/examples/web/jsf/ajaxguessnumber/` directory.
...
You will learn more about CDI in xref:introduction-to-jakarta-contexts-and-dependency-injection[].
