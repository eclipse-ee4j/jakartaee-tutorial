[[BNAWA]][[_delegating_rendering_to_a_renderer]]

== Delegating Rendering to a Renderer

Both `MapComponent` and `AreaComponent` delegate all of their rendering
to a separate renderer. The section
xref:jsf-custom/jsf-custom.adoc#BNAVW[Performing Encoding] explains how
...
* xref:jsf-custom/jsf-custom.adoc#BNAWB[Creating the Renderer Class]
* xref:jsf-custom/jsf-custom.adoc#BNAWC[Identifying the Renderer Type]

[[BNAWB]][[_creating_the_renderer_class]]
...
`AreaComponent` instance. xref:jsf-custom/jsf-custom.adoc#GLPBO[Configuring Model
...
[[BNAWC]][[_identifying_the_renderer_type]]
...
explained in xref:jsf-configure/jsf-configure.adoc#BNAXH[Registering a Custom
