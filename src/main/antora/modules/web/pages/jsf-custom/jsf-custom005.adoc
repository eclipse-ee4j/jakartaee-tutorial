...
As explained in xref:jsf-custom/jsf-custom.adoc#BNAVI[When to Use a Custom
...
xref:jsf-custom/jsf-custom.adoc#BNAWB[Creating the Renderer Class]. The behavior
...
xref:jsf-custom/jsf-custom.adoc#BNAWA[Delegating Rendering to a Renderer] for
more information.
...
`MapRenderer`. xref:jsf-custom/jsf-custom.adoc#BNAWD[Handling Events for Custom
Components] details how `MapComponent` handles events.

[[BNAVV]][[_specifying_the_component_family]]
...
configuration. xref:jsf-configure/jsf-configure.adoc#BNAXI[Registering a Custom
Component] explains how to define the component family in the component
configuration.

[[BNAVW]][[_performing_encoding]]
...
`encodeEnd` method of `AreaComponent`{zwsp}'s renderer after it invokes
`MapRenderer`{zwsp}'s `encodeBegin` method and before it invokes
`MapRenderer`{zwsp}'s `encodeEnd` method. If a component needs to perform the
...
[[BNAVY]][[_enabling_component_properties_to_accept_expressions]]
...
[[BNAVZ]][[_saving_and_restoring_state]]

=== Saving and Restoring State

As described in xref:jsf-custom/jsf-custom.adoc#BNAVY[Enabling Component Properties to Accept
