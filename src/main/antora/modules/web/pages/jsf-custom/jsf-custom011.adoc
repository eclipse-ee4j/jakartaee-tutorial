...
* Bind the `Converter` implementation to a managed bean property using the `f:converter` tag's `binding` attribute, as described in <<_binding_converters_listeners_and_validators_to_managed_bean_properties>>.
