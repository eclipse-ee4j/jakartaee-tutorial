[[GLPCB]][[_understanding_the_image_map_example]]
...
[[GLPBD]][[_why_use_javaserver_faces_technology_to_implement_an_image_map]]
...
[[GLPEM]][[_understanding_the_rendered_html]]
...
[[GLPCD]][[_understanding_the_facelets_page]]
...
xref:jsf-custom/jsf-custom.adoc#BNAWD[Handling Events for Custom Components].
...
xref:jsf-configure/jsf-configure.adoc#BNAXF[Configuring Navigation Rules].
...
xref:jsf-custom/jsf-custom.adoc#BNAVW[Performing Encoding].
...
xref:jsf-configure/jsf-configure.adoc#BNAWP[Application Configuration Resource
File].
...
of `ImageArea`. xref:jsf-custom/jsf-custom.adoc#BNAWB[Creating the Renderer
Class] explains how to do this in the `AreaRenderer` class.

[[GLPEL]][[_summary_of_the_image_map_application_classes]]

=== Summary of the Image Map Application Classes

xref:jsf-custom/jsf-custom.adoc#GLPEK[Table 15-2] summarizes all the classes needed to implement
