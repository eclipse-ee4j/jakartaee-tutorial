...
As explained in xref:jsf-custom/jsf-custom.adoc#_when_to_use_a_custom_component[When to Use a Custom Component], a component class defines the state and behavior of a UI component.
...
You will see how all this data is accessed through the value expression in xref:jsf-custom/jsf-custom.adoc#_creating_the_renderer_class[Creating the Renderer Class].
...
See xref:jsf-custom/jsf-custom.adoc#_delegating_rendering_to_a_renderer[Delegating Rendering to a Renderer] for more information.
...
xref:jsf-custom/jsf-custom.adoc#_handling_events_for_custom_components[Handling Events for Custom Components] details how `MapComponent` handles events.
