...
See xref:jsf-page-core/jsf-page-core.adoc#_using_the_standard_converters[Using the Standard Converters] for more information on these included converters.

As explained in xref:jsf-intro/jsf-intro.adoc#_conversion_model[Conversion Model], if the standard converters included with Jakarta Faces cannot perform the data conversion that you need, you can create a custom converter to perform this specialized conversion.
This implementation, at a minimum, must define how to convert data both ways between the two views of the data described in xref:jsf-intro/jsf-intro.adoc#_conversion_model[Conversion Model].
...
Alternatively, you can register the converter with entries in the application configuration resource file, as shown in xref:jsf-configure/jsf-configure.adoc#_registering_a_custom_converter[Registering a Custom Converter].
...
xref:jsf-configure/jsf-configure.adoc#_registering_application_messages[Registering Application Messages] explains how to register custom error messages with the application.
...
* Bind the `Converter` implementation to a managed bean property using the `f:converter` tag's `binding` attribute, as described in xref:jsf-custom/jsf-custom.adoc#_binding_converters_listeners_and_validators_to_managed_bean_properties[Binding Converters, Listeners, and Validators to Managed Bean Properties].
