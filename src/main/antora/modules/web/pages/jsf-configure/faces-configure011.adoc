...
When the application developer creates a custom renderer, as described in xref:jsf-custom/jsf-custom.adoc#_delegating_rendering_to_a_renderer[Delegating Rendering to a Renderer], you must register it using the appropriate render kit.
...
You register the renderer either by using the `@FacesRenderer` annotation, as described in xref:jsf-custom/jsf-custom.adoc#_creating_the_renderer_class[Creating the Renderer Class], or by using the `render-kit` element of the application configuration resource file.
