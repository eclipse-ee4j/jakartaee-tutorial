[[BNAXB]][[_registering_application_messages]]
...
described in xref:jsf-configure/jsf-configure.adoc#GKUHG[Using FacesMessage to Create a Message]
...
[[GKUHG]][[_using_facesmessage_to_create_a_message]]
...
[[BNASS]][[_referencing_error_messages]]

=== Referencing Error Messages

A Jakarta Server Faces page uses the `message` or `messages` tags to access
error messages, as explained in xref:jsf-page/jsf-page.adoc#BNASO[Displaying
