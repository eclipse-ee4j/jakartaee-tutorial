...
However, creating such a component is a significant task (see xref:jsf-custom/jsf-custom.adoc#_creating_custom_ui_components_and_other_custom_objects[Creating Custom UI Components and Other Custom Objects]).
...
The source code for this application is in the `_tut-install_/examples/web/jsf/reservation/` directory.
...
Next, an empty `h:head` tag followed by an `h:outputStylesheet` tag within the `h:body` tag illustrates the use of a relocatable resource (as described in xref:jsf-facelets/jsf-facelets.adoc#_relocatable_resources[Relocatable Resources]):
...
See xref:jsf-ajax/jsf-ajax.adoc#_using_ajax_with_jakarta_faces_technology[Using Ajax with Jakarta Faces Technology], for more information.
