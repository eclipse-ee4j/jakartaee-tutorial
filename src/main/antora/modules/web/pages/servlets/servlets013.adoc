...
<<_asynchronous_processing>> provides a mechanism to execute application-specific blocking operations in a new thread, returning the thread associated with the request immediately to the container.
...
. Put the request in asynchronous mode as described in <<_asynchronous_processing>>.
