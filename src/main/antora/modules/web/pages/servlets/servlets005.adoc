[[BNAFV]][[_writing_service_methods]]

== Writing Service Methods

The service provided by a servlet is implemented in the `service` method
of a `GenericServlet`, in the `do`{zwsp}Method methods (where Method can take
...
[[BNAFW]][[_getting_information_from_requests]]
...
See xref:webi18n/webi18n.adoc#BNAXU[Chapter 22, "Internationalizing and
...
tracking a user's session (see xref:servlets/servlets.adoc#BNAGR[Session
Tracking]).
