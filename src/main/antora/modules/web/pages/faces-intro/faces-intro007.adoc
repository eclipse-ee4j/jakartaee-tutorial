...
For example, Ajax applications use partial processing features of the lifecycle (see xref:jsf-intro/jsf-intro.adoc#_partial_processing_and_partial_rendering[Partial Processing and Partial Rendering]).
A clearer understanding of the lifecycle phases is key to creating well-designed components.

A simplified view of the Jakarta Faces lifecycle, consisting of the two main phases of a Jakarta Faces web application, is introduced in xref:jsf-intro/jsf-intro.adoc#_a_simple_jakarta_faces_application[A Simple Jakarta Faces Application].
