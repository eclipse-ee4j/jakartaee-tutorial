...
This example is based on the unsecured enterprise bean application, `converter`, which is discussed in xref:getting-started-with-enterprise-beans[] and is found in the `_tut-install_/examples/ejb/converter/` directory.
