...
. Use Jakarta Faces and Jakarta Validation to customise the Form used for Form authentication
. How to define (and implicitly set) a custom https://jakarta.ee/specifications/security/3.0/apidocs/jakarta.security/jakarta/security/enterprise/identitystore/package-summary.html[identity store,role=external,window=_blank]
include::partial$list-item-learn-security-context.adoc[]
...
To use this authentication method, we need to designate a path to a resource that is relative to our application. The authentication mechanism will redirect the caller to this resource when authentication is required. The resource can be anything, but a postback should eventually lead to some code being executed that continues the authentication dialog. E.g. a plain .html file or .jsp file combined with  a Filter, or a Faces view with a backing bean.
        ...
        <p>
         ...
         <form faces:id="form">
            <p>
                ...
                <input faces:id="username" type="text" faces:value="#{loginBacking.username}" />
            </p>
            <p>
                <strong>Password </strong> 
                <input faces:id="password" type="password" faces:value="#{loginBacking.password}" />
            </p>
            <p>
                <input type="submit" value="Login" faces:action="#{loginBacking.login}" />
            </p>
        </form>
    </body>
</html>
...
If this initial validation passes, we arrive in the `login()` method. For our example the only thing we need to do here is signaling that we want to continue the authentication dialog, and when doing so provide the credentials that we earlier obtained in a custom way.
