...
8. Add this module to the `src/main/antora/antora.yml`. For example, for the Security module, you would add this line to the end of  `src/main/antora/antora.yml` file:

```yaml
  - modules/security/nav.adoc
```

9. Check the links. For any links that refer to anchors in other files (usually within the same folder) you will need to fix them manually. For example, in the Security module, this link:
...
This is necessary because the entire module isn't a single file.  Note the use of the `xref` here instead of `link`. We're referring to another folder within the `security` module's `pages` folder. (See the https://docs.antora.org/antora/latest/page/xref/[the Antora docs] for more info about xrefs.)
